...
class Wav2Vec2EncoderStableLayerNorm(nn.Module):
    def __init__(self, config):
        ...
        self.num_scc_layers = config.num_scc_layers
        self.compress_ratio = self.num_hidden_layers // self.num_scc_layers
        ...
        self.layers = nn.ModuleList(
            [Wav2Vec2EncoderLayerStableLayerNorm(config) for _ in range(self.num_hidden_layers)]
        )
        self.scc_layers = nn.ModuleList(
            [Wav2Vec2EncoderLayerStableLayerNorm(config) for _ in range(self.num_scc_layers)]
        )
        self.gradient_checkpointing = False
    ...
    def forward(
        ...
        if self.training:
            inference_layers = []
            for i in range(self.num_scc_layers):
